...
fn test_validate_heuristics_file() {
    ...
    let content = fs::read_to_string("heuristics.toml")
    ...
    assert!(rules_value.get("rules").is_some(), "Should have 'rules' array");
...
fn test_all_rules_have_required_fields() {
    ...
    for (idx, rule) in rules.iter().enumerate() {
        let rule_table = rule.as_table()
            .unwrap_or_else(|| panic!("Rule {} is not a table", idx));

        // Required fields for all rules
        assert!(rule_table.contains_key("name"), "Rule {} missing 'name'", idx);
        assert!(rule_table.contains_key("type"), "Rule {} missing 'type'", idx);
        assert!(rule_table.contains_key("risk_score"), "Rule {} missing 'risk_score'", idx);
        assert!(rule_table.contains_key("description"), "Rule {} missing 'description'", idx);
        ...
        match rule_type {
            "keyword" => {
                assert!(rule_table.contains_key("field"), "Keyword rule {} missing 'field'", idx);
                assert!(rule_table.contains_key("keywords"), "Keyword rule {} missing 'keywords'", idx);
            }
            "regex" => {
                assert!(rule_table.contains_key("field"), "Regex rule {} missing 'field'", idx);
                assert!(rule_table.contains_key("pattern"), "Regex rule {} missing 'pattern'", idx);
            }
            "metadata" => {
                assert!(rule_table.contains_key("field"), "Metadata rule {} missing 'field'", idx);
                assert!(rule_table.contains_key("check"), "Metadata rule {} missing 'check'", idx);
            }
...
fn test_risk_scores_in_valid_range() {
    ...
    for rule in rules {
        let risk_score = rule.get("risk_score")
            .expect("Missing risk_score")
            .as_integer()
            .expect("risk_score is not an integer");

        assert!((0..=100).contains(&risk_score),
                "Risk score {} out of range (0-100)", risk_score);
    }
}
...
fn test_valid_field_names() {
    ...
    for (idx, rule) in rules.iter().enumerate() {
        if let Some(field) = rule.get("field") {
            ...
            assert!(valid_fields.contains(&field_str),
                    "Rule {} has invalid field name: {}", idx, field_str);
        }
    }
}
...
fn test_metadata_checks_valid() {
    ...
    for rule in rules {
        if rule.get("type").unwrap().as_str().unwrap() == "metadata" {
            let check = rule.get("check")
                .expect("Metadata rule missing 'check'")
                .as_str()
                .expect("check is not a string");

            assert!(valid_checks.contains(&check),
                    "Invalid metadata check: {}", check);
        }
    }
}
